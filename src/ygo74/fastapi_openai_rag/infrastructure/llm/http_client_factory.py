"""HTTP client factory for enterprise environments with proxy and SSL support."""
import ssl
import os
import urllib.parse
import httpx
...
class HttpClientFactory:
    ...
    def create_async_client(
        ...
        **kwargs
        ...
        """Create async httpx client with enterprise configuration.
        ...
        limits = httpx.Limits(
            max_connections=200,  # Total concurrent connections across all hosts
            max_keepalive_connections=40,  # Keep more connections alive for reuse
    ...
    def create_sync_client(
        ...
        **kwargs
        ...
        """Create sync httpx client with enterprise configuration.
        ...
        Returns:
            httpx.Client: Configured sync HTTP client
        """
        # Configure SSL context for enterprise environments
        ssl_context = HttpClientFactory._configure_ssl_context(
            verify_ssl, ca_cert_file, client_cert_file, client_key_file
    ...
    def _get_proxy_from_env(target_url: str) -> Optional[httpx.Proxy]:
        ...
        if HttpClientFactory._should_bypass_proxy(target_url):
            logger.debug("Target URL matches no_proxy pattern, bypassing proxy")
            return None

        # Determine which proxy to use based on target URL scheme
        target_scheme = urllib.parse.urlparse(target_url).scheme.lower() if target_url else "https"
        ...
        for env_var in proxy_env_vars:
            ...
            if proxy_url:
                ...
                proxy_auth = HttpClientFactory._parse_proxy_auth(proxy_url)

                # Remove auth from URL if present (httpx handles it separately)
                clean_proxy_url = HttpClientFactory._clean_proxy_url(proxy_url)
